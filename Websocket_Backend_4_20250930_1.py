...
FORWARDED_OPTS = {"--index-url", "--extra-index-url", "--find-links",
                  "--trusted-host", "--pre"}
...
def parse_requirements(req_path: Path):
    """Return (global_opts:list[str], packages:list[str]) from a requirements.txt."""
    global_opts, packages = [], []
    for raw in req_path.read_text().splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        if line.startswith(("-r", "--requirement")):
            inc = line.split(maxsplit=1)[1]
            inc_path = (req_path.parent / inc).resolve()
            g2, p2 = parse_requirements(inc_path)
            global_opts += g2
            packages += p2
            continue
        if line.startswith("-"):
            # keep only safe/global pip options to forward each time
            parts = shlex.split(line)
            if parts and parts[0] in FORWARDED_OPTS:
                global_opts += parts
            continue
        packages.append(line)
    return global_opts, packages

def install_one_by_one(req_file: str):
    req_path = Path(req_file)
    opts, specs = parse_requirements(req_path)
    if not specs:
        print(f"No packages found in {req_path}")
        return True

    all_ok = True    
    i=2
    for spec in specs:
        print(f">> Installing: {spec} …")  # shows up in syslog
        i=i+1
        try: # attempting to put something on screen without any library
            subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "50", "--y", str(i*20), "--t", "update "+str(spec).replace("'",""), "--f", "2"],check=True)
        except:
            pass # nevermind...
        cmd = [sys.executable, "-m", "pip", "install",
               "--no-input", "--retries", "5", "--timeout", "30"] + opts + [spec]
        # inherit stdout/stderr so pip logs stream to syslog in real time
        proc = subprocess.run(cmd)
        if proc.returncode != 0:
            all_ok = False
            print(f"!! Failed: {spec} (exit {proc.returncode})")
            try: # attempting to put something on screen without any library
                subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "5", "--y", str(i*20), "--t", "!X", "--f", "2"],check=True)
            except:
                pass # nevermind...
        else:
            print(f">> Done: {spec}")
            try: # attempting to put something on screen without any library
                subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "5", "--y", str(i*20), "--t", "OK", "--f", "2"],check=True)
            except:
                pass # nevermind...
...
def ensure_requirements():
    ...
    try: # attempting to put something on screen without any library
        ...
        subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "50", "--y", "20", "--t", "Updating dependencies", "--f", "2"],check=True)
    except:
        pass # nevermind...
    ...
    if not wait_for_dns():
        ...
        try: # attempting to put something on screen without any library
            ...
            pass
        SCREEN_LOCK=False # releases screen
        return False
    try:
        #subprocess.check_call([sys.executable, "-m", "pip", "install", "-r", str(REQ_FILE)])
        # here we update directly from the file
        ...
        # here I update one element at a time so I can post a message in the process
        install_one_by_one(REQ_FILE)
    ...
    try:
        subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "240", "--t", "Done!", "--f", "3"],check=True)
    except:
        pass
    SCREEN_LOCK=False
    # mark as satisfied
...
# we need the following from 30th Sept 2025 to try and extract
# SVG files dimensions
...
try:
    ...
    print("Failed to import GerberFile from gerbonara")
...
try:
    import local_packages.GerberTranslator_1_4_4_integrated as Translator
except Exception as e:
...
try:
    import local_packages.SVGTranslator_5_5_2 as SVGTranslator
except Exception as e:
...
# used for storing data for SVG (and later Gerber) pageSetup
setupPageData={}
    ...
    try:
        ...
        try:
            ...
            try: # attempting to put something on screen without any library
                ...
                pass # nevermind...
        try:
            subprocess.run(["/sbin/shutdown", "-r", "now"], check=True)
        except Exception as e:
...
def translateFile(data=""):
    ...
    try:
        if(ConfigDict['SVG']=="True"):
            ...
            # how do I pass the setupPageData JSON to SVGTranslator?
            # They have been added to ConfigDict
            '''
            Added key units to ConfigDict with value:  mm
            Added key image to ConfigDict with value:  {'width': 100, 'height': 50}
            Added key printArea to ConfigDict with value:  {'width': 210, 'height': 297}
            Added key margins to ConfigDict with value:  {'top': 10, 'right': 10, 'bottom': 10, 'left': 10}
            Added key scalePercent to ConfigDict with value:  100
            Added key alignment to ConfigDict with value:  middle-center
            Added key placement to ConfigDict with value:  {'scaledWidth': 100, 'scaledHeight': 50, 'x': 55, 'y': 123.5}
            '''           
...
def setupPage(data=""):
    #### NOTE: it works both for SVG and Gerber
    ## BUT Gerbers do not sue it in the traslation phase
    global setupPageData
    setupPageData={}
    if(data.find("{")>0): # removes the initial part of the data leaving only the {...}
        data=data[data.find("{"):]
    else:
        print("Unexpected data format!")
    print(">>> setupPage invoked with param: ",data)
    # transforming into a JSON so we can extract keys
    step1 = data.encode("utf-8").decode("unicode_escape")
    setupPageData=json.loads(step1)
    # now we have a set of keys:
    # dict_keys(['units', 'image', 'printArea', 'margins', 'scalePercent', 'alignment', 'placement'])
    print("setupPageData variable is set")
    # how do I use these parameters to define the conversion?
    # do I add them to the ConfigDict (that is passed to the Translator)?
    for each in setupPageData.keys():
        ConfigDict[each]=setupPageData[each]
        try:
            print(f"Added key {each} to ConfigDict with value: ",ConfigDict[each])
        except:
            pass
...
def setTranslatorParameters(data=""):
    ...
    if ('SVG') in fields and fields['SVG']=="True" :
        try: # sets the name of the putput file equal to the input but changing extension
    ...
    width, height = None , None # dimensions of SVG and GERBER will go here
    # if SVG in fields, we could try to extract canvas dimensions...
    if ('SVG') in fields and fields['SVG']=="True" :
        # identify full path name of the SVG file that should be in
        SVGFullFileName=Path(ConfigDict['RootDirectory']) / fields['SVGFileName']
        #print("Attempting to get dimensions from file: ",str(SVGFullFileName))
        try:
            file_in=open(SVGFullFileName,'r')
            tree = ET.parse(file_in)
            file_in.close()
            #print("File has been opened and tree extracted")
        except Exception as e:
            print(f"Exception while opening {FullInFileName}")
            print("Exception is: ",e)
        # now we try and extract the dims
        try:
            root = tree.getroot()
            print("Extracting canvas dimensions..")
            width = root.get('width')
            height = root.get('height')
            print(f".. from root W={width} and H={height}")
            if width == None or height == None:
                viewbox = root.get('viewBox')
                if viewbox:
                    _, _, width, height = viewbox.split()
                    print(f".. from viewbox W={width} and H={height}")            
            if width == None or height == None:
                my_print ("Unable to get width and height for the svg")
        except Exception as e:
            print("Exception while processing SVG file data tree: ",e)
    #print("breakpoint 1 reached")

    ### addition 1st October 2025
    # if GERBER in fields, we could try to extract canvas dimensions...
    # with Gerber the board outline is defined in a separate file
    # but I can use gerbonara library to extract the bounding box
    if ('GERBER') in fields and fields['GERBER']=="True" :
        GERBERFullFileName=Path(ConfigDict['RootDirectory']) / fields['GerberFileName']
        #print("Gerber full file name is: ", GERBERFullFileName)
        xmin, ymin, xmax, ymax = inf, inf, -inf, -inf
        print("breakpoint 2 reached")
        try:
            g = GerberFile.open(GERBERFullFileName)
            for obj in g.objects:                           # objects = lines, arcs, flashes, regions...
                (x0, y0), (x1, y1) = obj.bounding_box('mm') # returns ((minx,miny),(maxx,maxy))
                xmin, ymin = min(xmin, x0), min(ymin, y0)
                xmax, ymax = max(xmax, x1), max(ymax, y1)
            width, height = str(int((xmax - xmin)*100+0.5)/100.0), str(int((ymax - ymin)*100+0.5)/100.0) # to maintain correct format
            width=width+"mm" #recovering correct data format
            height=height+"mm"
            print("Bounding box extracted from gerber file")
        except Exception as e:
            print("Failed to extract bounding box via gerbonara: ",e)
            
    if not (width == None or height == None): # we have dimensions
        # we need to separate dimensions from numbers
        unit="" 
        try:
            if (width.find("mm")>0):
                width=width[0:width.find("mm")]
                height=height[0:height.find("mm")]
                unit="mm"
            if (width.find("in")>0): # converts everything in mm
                width=width[0:width.find("in")]
                height=height[0:height.find("in")]
                unit="in"
        except:
            pass
        # we now send a message to the FrontEnd with the
        # canvas dimension data to be used by the setPage form
        # SEND_DATA_TO_FRONTEND
        msg_loc={ "CANVAS_W":width, "CANVAS_H":height, "CANVAS_UNITS":unit}
        json_data=json.dumps(msg_loc)
        print("Sending to FrontEnd: CANVAS_DIMS: "+json_data)
        try:
            FrontEndQueue.put_nowait("CANVAS_DIMS: "+json_data)
        except Exception as e:
            print("Failed Sending to FrontEnd: ",e)
...
def CheckAndUpdate():
    ...
    response=GetAvailableFile() # it is a list with the names of the packages available for download
    Updates=[]
...
#             print("Extension is: ",extension)
    ...
    if CodeToRelink or ZipToRefresh:
        try:
            try:
        ...
        try:
            try: # attempting to put something on screen without any library
                subprocess.run(["/home/pi/fbdisplay/clearscreen"],check=True)
                subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "10", "--t", "Restarting ", "--f", "3"],check=True)
                subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "50", "--t", "Services...", "--f", "3"],check=True)
            except:
                pass # nevermind...
            restart_self_systemd()
        except Exception as e:
            ...
            try: # attempting to put something on screen without any library
                subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "90", "--t", "... failed", "--f", "3"],check=True)
                subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "130", "--t", "trying reboot", "--f", "3"],check=True)
            except:
                pass # nevermind...
            try:
                subprocess.run(["/sbin/shutdown", "-r", "now"], check=True)
            except:
                print(f"System shutdonw -r failed")
                try: # attempting to put something on screen without any library
                    subprocess.run(["/home/pi/fbdisplay/clearscreen"],check=True)
                    subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "10", "--t", "Reboot failed!", "--f", "3"],check=True)
                    subprocess.run(["/home/pi/fbdisplay/fbtext", "--x", "20", "--y", "50", "--t", "Turn off", "--f", "3"],check=True)
...
#             print("Exception while running fixPackages: ",e)
    ...
    print(TextSsid)
    print(TextIp)
    # I could use the SCREEN_LOCK variable, that is set to True when other parts
    # are using the screen to avoid conflicts...
